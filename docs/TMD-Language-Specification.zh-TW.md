...
- **首調音高基準（`?=`）**：
  設定簡譜唱名 $1$ 所對齊的音高基準（例如 `?= C` 代表 $1=C$、`?= D` 代表 $1=D$）。不在 parser 層驗證調號是否合理，純粹作為音高平移（Key Offset）依據。
- **明確調性宣告（`key=` 或 `Key=`）**：
  用於正式宣示樂曲的調性與大小調模式（如 `key= Bm`、`key= C`、`key= F#m`、`Key= Am`）。供樂譜匯出（如 MusicXML 的 `<mode>` 與調號記號）、真人樂手演奏約定，以及 Song Inspector 的調性吻合度驗證。調性轉換 `{?+5}` 是播放順序中的相對轉調，段落內亦可使用 `{key= Bm}` 明確切換當前調性。

目前 Song Inspector 的調性分析以大調與小調模型為實作基準。產品規劃上優先支援
大調與小調，這已能涵蓋多數流行歌曲；Dorian、Mixolydian 等其他調式可列為後續
延伸，未完整支援前不應將分析結果解讀為正式的調式判定。
...
- 絕對速度（`{!=140}`）與相對速度（`{!+10}`）
- 絕對首調基準（`{?=D}`）與相對移調（`{?+2}`、`{?-2}`）
- 明確調性切換（`{key= Bm}` 或 `{Key= Bm}`）
- 力度標記（`{pp}`, `{p}`, `{mp}`, `{mf}`, `{f}`, `{ff}` 等）：設定後續音符的播放 Velocity 力度與樂譜排版強弱記號
