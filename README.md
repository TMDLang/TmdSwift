...
2. **Instant Sensory Feedback Loop**: Inspect vocal tessitura (`tmd inspect`), lint measure rhythm math (`tmd check`), and render offline audio (`tmd input.tmd -w song.wav`) in seconds directly from the command line. Tweak each part in a rapid, tight verification loop.
...
### Swift Library Localization

User-facing reports generated by `TmdSwift` use a resource-backed localization catalog rather than language branches in the analysis code. The default locale is Traditional Chinese, and English is available explicitly:
...
`TMDLocale` is string-backed, so adding a future language only requires adding a matching `Sources/TmdSwift/Resources/<locale>.lproj/Localizable.strings` catalog. Missing translations fall back to the configured fallback locale.
